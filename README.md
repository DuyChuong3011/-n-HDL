...
Quy trình chạy tự động hóa bằng `Makefile` và yêu cầu chạy lệnh `make` hoặc `mingw32-make`.
...
1.  **Pha 1 (Input):** Python (`img2mem.py`) chuyển đổi ảnh --> RGB --> Độn viền --> Xuất ra file `input_test.mem` (24-bit HEX).
2.  **Pha 2 (Verilog):** ModelSim chạy Pha Ghi (Store) --> Pha Xử lý (Rotate) --> Ghi kết quả 24-bit vào `output_transformed.mem`.
3.  **Pha 3 (Output):** Python (`mem2png.py`) đọc 24-bit HEX --> Tái tạo và lưu ảnh màu **RGB** kết quả.
...
| Lệnh             | Chức năng                                                                  |
| :---             | :---                                                                       |
| **`make clean`** | Xóa tất cả các file trung gian và output (`.mem`, `.png`).                 |
